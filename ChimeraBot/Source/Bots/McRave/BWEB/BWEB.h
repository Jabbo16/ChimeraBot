...
namespace BWEB
{
	...
	private:
		...
		// Walls
		bool isWallTight(BWEBWall&, UnitType, TilePosition);
		bool isPoweringWall(BWEBWall&, TilePosition);
		bool iteratePieces(BWEBWall&);
		void findCurrentHole(BWEBWall&, bool);

		TilePosition initialStart, initialEnd;
		void initializePathPoints(BWEBWall&);
		void checkPathPoints(BWEBWall&);
	...
	public:
		...
		BWEBWall* getWall(BWEM::Area const* area = nullptr, BWEM::ChokePoint const* choke = nullptr);
		...
		vector<BWEBWall> getWalls() const { return walls; }
		...
		const BWEBWall* getClosestWall(TilePosition) const;
		...
		void addToWall(UnitType type, BWEBWall& wall, UnitType tight = UnitTypes::None);
