...
namespace BWEB
{
	BWEBWall::BWEBWall(const BWEM::Area * a, const BWEM::ChokePoint * c, vector<UnitType> b, vector<UnitType> d)
	{
		area = a;
		choke = c;
	...
	void BWEBWall::insertSegment(const TilePosition here, UnitType building)
	{
	...
	void Map::createWall(vector<UnitType>& buildings, const BWEM::Area * area, const BWEM::ChokePoint * choke, const UnitType tight, const vector<UnitType>& defenses, const bool reservePath, const bool requireTight)
	{
		...
		BWEBWall wall(area, choke, buildings, defenses);
	...
	bool Map::iteratePieces(BWEBWall& wall)
	{
		...
		const auto testPiece = [&](BWEBWall& wall, TilePosition t) {
	...
	bool Map::isPoweringWall(BWEBWall& wall, const TilePosition here)
	{
		for (auto &piece : currentWall) {
	...
	bool Map::isWallTight(BWEBWall& wall, UnitType building, const TilePosition here)
	{
	...
	void Map::checkPathPoints(BWEBWall& wall)
	{
	...
	void Map::initializePathPoints(BWEBWall& wall)
	{
	...
	void Map::findCurrentHole(BWEBWall& wall, bool ignoreOverlap)
	{
	...
	void Map::addToWall(UnitType building, BWEBWall& wall, UnitType tight)
	{
	...
	const BWEBWall * Map::getClosestWall(TilePosition here) const
	{
		double distBest = DBL_MAX;
		const BWEBWall * bestWall = nullptr;
		for (auto &wall : walls) {
	...
	BWEBWall* Map::getWall(const BWEM::Area * area, const BWEM::ChokePoint * choke)
	{
		if (!area && !choke)
			return nullptr;
